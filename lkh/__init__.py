...
def solve(solver="LKH", problem=None, **params):
    ...
    # Extract the cost from the tour file.
    with open(params["tour_file"], "r") as fh:
        solution_data = fh.read()

    pattern = "Length = "
    start = solution_data.find(pattern)
    end = start + 1
    while end < len(solution_data) and solution_data[end] != "\n":
        end += 1

    cost = int(solution_data[start + len(pattern) : end])
